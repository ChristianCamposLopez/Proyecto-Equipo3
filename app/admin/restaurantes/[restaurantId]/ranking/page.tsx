...
import { ExportService, ExportData, ExportMetadata } from '@/lib/export.service';
...
export default function RankingPage() {
  ...
  const fetchRanking = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || 'Error al obtener el ranking');
      setRanking(result.data || []);
  ...
  return (
    <>
      ...
      <div className="edit-root">
        ...
        <div className="edit-footer">
          <span>{restaurantName || `Restaurante ID: ${restaurantId}`}</span>
          <span>Ranking actualizado en tiempo real</span>
        </div>
      </div>
    </>
