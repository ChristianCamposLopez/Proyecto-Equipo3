// controllers/statsController.ts — US019: Estadísticas de platos más vendidos
import { NextRequest, NextResponse } from 'next/server';
import { StatsDAO } from '@/models/daos/StatsDAO';

const dao = new StatsDAO();
...
export async function getTopProducts(
  ...
  try {
    ...
    const products = await dao.getTopProducts(restaurantId, days, limit);
...
export async function getDailySalesStats(
  ...
  try {
    ...
    const stats = await dao.getDailySalesStats(restaurantId, days);
...
export async function getRevenueByCategory(
  ...
  try {
    ...
    const revenue = await dao.getRevenueByCategory(restaurantId, days);
...
export async function getSalesOverview(
  ...
  try {
    ...
    const overview = await dao.getSalesOverview(restaurantId, days);
