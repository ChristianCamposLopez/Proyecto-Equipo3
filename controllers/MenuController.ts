...
export class MenuController {
  ...
  async createProduct(data: {
    ...
    const product = await ProductoDAO.createProduct(data);
    return product;
  }

  // Actualizar nombre/descripción
  async updateProduct(id: number, data: { name?: string; descripcion?: string }): Promise<any> {
    const product = await ProductoDAO.findByIdIncludingInactive(id);
    if (!product) {
      throw new Error('Producto no encontrado');
    }
    ...
    return await ProductoDAO.updateProduct(id, data);
  }
  ...
  async updateNombre(id: number, name: string) {
    ...
    const updated = await ProductoDAO.updateProduct(id, { name });
    return updated;
  }
