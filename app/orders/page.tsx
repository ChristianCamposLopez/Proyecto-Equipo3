...
export default function OrdersPage() {
  ...
  const loadData = useCallback(async (options?: { showLoading?: boolean; preserveMessage?: boolean }) => {
    ...
    try {
      const [ordersRes, cartRes, addressesRes] = await Promise.all([
        fetch(`/api/orders?customerId=${DEMO_CUSTOMER_ID}`),
        fetch(`/api/cart?customerId=${DEMO_CUSTOMER_ID}`),
        fetch(`/api/delivery-addresses?customerId=${DEMO_CUSTOMER_ID}`),
      ])
  ...
  const removeItem = async (itemId: number) => {
    ...
    setMessage(null)
    try {
      await syncCart(`/api/cart/items/${itemId}?customerId=${DEMO_CUSTOMER_ID}`, {
        method: "DELETE",
      })
  ...
  const clearCurrentCart = async () => {
    setMessage(null)
    try {
      await syncCart(`/api/cart?customerId=${DEMO_CUSTOMER_ID}`, {
        method: "DELETE",
      })
            ...
            <div className="hero-grid">
              <div>
                ...
                <div className="hero-actions">
                  <a className="primary-btn" href="/menu">Seguir comprando</a>
