...
export class DisponibilidadDAO {
  static async getByProductId(productId: number): Promise<Disponibilidad[]> {
  ...
  static async getById(id: number): Promise<Disponibilidad | null> {
    const result = await db.query('SELECT * FROM product_availability WHERE id = $1', [id]);
  ...
  static async delete(id: number): Promise<boolean> {
    const result = await db.query('DELETE FROM product_availability WHERE id = $1', [id]);
